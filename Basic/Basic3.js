 ...
 */
 function doAlphaBlending(index, images, alphas) {
    ...
    let resultColor = [255, 255, 255]; // [r, g, b]
    let accumulatedAlpha = 1.0; // Tracks the remaining transparency after each layer.

    // 2. Loop over the four circle images in the correct order to blend
    //    them one after another.

    for (let i = 0; i < images.length; i++) {
        // Extract the alpha value of the current pixel in the image.
        let imageAlpha = images[i][index + 3] / 255; // Normalize the alpha to [0, 1].

        // Skip blending if the pixel is transparent in the image.
        if (imageAlpha === 0) continue;

        // Combine the image alpha with the slider alpha.
        let effectiveAlpha = alphas[i] * imageAlpha;
        ...
        for (let channel = 0; channel < 3; channel++) {
            resultColor[channel] =
                effectiveAlpha * images[i][index + channel] +
                (1 - effectiveAlpha) * resultColor[channel];
        }

        // 4. Compute the resulting color using alpha blending in all
        //    three color channels.
        accumulatedAlpha *= (1 - effectiveAlpha);

         // If no transparency remains, stop blending.
         if (accumulatedAlpha <= 0) break;
        }

    // 5. Return the resulting color. Replace the following dummy line.
    return resultColor.map(Math.round);
}
