...
mat4.perspective = function (out, fovy, near, far) {
    ...
    const f = 1.0 / Math.tan(fovy / 2); // Cotangent of half the field of view
    ...
    out[8] = 0;
    out[9] = 0;
    out[10] = (far + near) * nf;   // Perspective depth
    out[11] = -1;

    out[12] = 0;
    out[13] = 0;
    out[14] = (2 * far * near) * nf; // Depth offset
    out[15] = 0;
...
class Camera3D {
    ...
    update() {
       ...
       // Calculate w (lookAt direction)
    this.w = vec3.create();
    vec3.sub(this.w, this.eye, this.lookAtPoint);
    vec3.normalize(this.w, this.w);

    // Calculate u (side vector)
    this.u = vec3.create();
    vec3.cross(this.u, this.upVector, this.w);
    vec3.normalize(this.u, this.u);

    // Calculate v (up vector in the camera's space)
    this.v = vec3.create();
    vec3.cross(this.v, this.w, this.u);

    // Set up the camera matrix (world to camera transformation)
    this.cameraMatrix = mat4.fromValues(
        this.u[0], this.v[0], this.w[0], 0,
        this.u[1], this.v[1], this.w[1], 0,
        this.u[2], this.v[2], this.w[2], 0,
        -vec3.dot(this.u, this.eye), -vec3.dot(this.v, this.eye), -vec3.dot(this.w, this.eye), 1
    );

    // Update projection matrix using mat4.perspective
